...
module Nanoc
  ...
  # Nanoc::DependencyTracker records the dependencies between pages, assets
  ...
  class DependencyTracker
    ...
    def start
      ...
      Nanoc::NotificationCenter.on(:visit_started, :dependency_tracker) do |obj|
        @stack.push(obj_to_id(obj))
      end

      # Register end of visits
      Nanoc::NotificationCenter.on(:visit_ended, :dependency_tracker) do |obj|
    ...
    def stop
      # Unregister
      Nanoc::NotificationCenter.remove(:visit_started, :dependency_tracker)
      Nanoc::NotificationCenter.remove(:visit_ended,   :dependency_tracker)
    end
      ...
      while anything_changed do
        ...
        reps.each do |rep|
          ...
          dependencies = dependencies_raw.map do |dependency|
            reps.find do |rep|
              rep.item.identifier == dependency[1] && (rep.is_a?(Nanoc::PageRep) ? :page : :asset) == dependency[0]
            end
          end
