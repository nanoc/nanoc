...
class Nanoc::OrigCLITest < Nanoc::TestCase
  ...
  def test_load_command_at_with_non_utf8_encoding
    ...
    Nanoc::OrigCLI.load_command_at(root_dir + '/lib/nanoc/orig_cli/commands/create-site.rb')
  ensure
  ...
  def test_enable_utf8_only_on_tty
    ...
    with_env_vars(new_env_diff) do
      ...
      refute Nanoc::OrigCLI.enable_utf8?(io)

      io = StringIO.new
      def io.tty?
        false
      end
      assert Nanoc::OrigCLI.enable_utf8?(io)
    end
  end
  ...
  def test_enable_utf8
    ...
    with_env_vars(new_env_diff) do
      ...
      with_env_vars('LC_ALL'   => 'en_US.UTF-8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
      with_env_vars('LC_CTYPE' => 'en_US.UTF-8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
      with_env_vars('LANG'     => 'en_US.UTF-8') { assert Nanoc::OrigCLI.enable_utf8?(io) }

      with_env_vars('LC_ALL'   => 'en_US.utf-8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
      with_env_vars('LC_CTYPE' => 'en_US.utf-8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
      with_env_vars('LANG'     => 'en_US.utf-8') { assert Nanoc::OrigCLI.enable_utf8?(io) }

      with_env_vars('LC_ALL'   => 'en_US.utf8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
      with_env_vars('LC_CTYPE' => 'en_US.utf8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
      with_env_vars('LANG'     => 'en_US.utf8') { assert Nanoc::OrigCLI.enable_utf8?(io) }
    end
  end
end
